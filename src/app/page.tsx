  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 overflow-x-hidden">
      ...
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-100">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2.5 group">
            ...
            <span className="font-display text-xl font-bold text-slate-900 tracking-tight">Finanza</span>
          </Link>
          <nav className="flex items-center gap-3">
            <Link href="/login" className="text-sm font-medium text-slate-600 hover:text-slate-900 px-4 py-2 rounded-lg hover:bg-slate-100 transition-colors">
              Entrar
            </Link>
      ...
      <section className="relative py-24 md:py-32 overflow-hidden">
        ...
        <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-blue-50 rounded-full blur-3xl opacity-60" style={{ transform: 'translate(25%, -25%)' }} />
        <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-emerald-50 rounded-full blur-3xl opacity-50" />

        <div className="relative max-w-6xl mx-auto px-6 grid md:grid-cols-2 gap-16 items-center">
          <div>
            <div className="anim-fade-up inline-flex items-center gap-2 bg-blue-50 border border-blue-100 text-blue-700 text-xs font-semibold px-3 py-1.5 rounded-full mb-6">
              <span className="w-1.5 h-1.5 bg-blue-500 rounded-full pulse-dot" />
              Gestão financeira pessoal
            </div>
            <h1 className="font-display anim-fade-up-1 text-5xl md:text-6xl font-extrabold text-slate-900 leading-tight tracking-tight mb-6">
              Controle suas{' '}
              <span className="text-blue-600">finanças</span>
              <br />com inteligência
            </h1>
            <p className="anim-fade-up-2 text-lg text-slate-500 leading-relaxed mb-10 max-w-md">
            ...
            <div className="anim-fade-up-3 flex flex-wrap gap-3">
              ...
              <Link href="/login" className="inline-flex items-center gap-2 bg-white hover:bg-slate-50 text-slate-800 font-semibold px-6 py-3 rounded-xl border border-slate-200 shadow-sm transition-all hover:-translate-y-0.5">
                Entrar
              </Link>
            </div>
          ...
          <div className="anim-fade-in relative hidden md:flex justify-center items-center">
            <div className="float-card relative w-full max-w-sm">
              <div className="glass-card rounded-2xl shadow-2xl shadow-blue-100 p-6 space-y-5">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs text-slate-400 font-medium uppercase tracking-wider">Saldo atual</p>
                    <p className="font-display text-3xl font-bold text-slate-900 mt-0.5">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-emerald-50 rounded-xl p-3.5">
                    <div className="flex items-center gap-1.5 mb-1">
                      ...
                      <span className="text-xs font-medium text-emerald-700">Receitas</span>
                    </div>
                    <p className="font-display text-lg font-bold text-emerald-700">R$&nbsp;7.500</p>
                  </div>
                  <div className="bg-red-50 rounded-xl p-3.5">
                    ...
                    <p className="font-display text-lg font-bold text-red-600">R$&nbsp;3.219</p>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  {[
                    { label: 'Supermercado', cat: 'Alimentação', amount: '-R$ 245', color: 'bg-orange-100 text-orange-600' },
                    { label: 'Salário', cat: 'Receita', amount: '+R$ 6.000', color: 'bg-emerald-100 text-emerald-600' },
                    { label: 'Netflix', cat: 'Lazer', amount: '-R$ 55', color: 'bg-pink-100 text-pink-600' },
                  ].map((tx) => (
                    <div key={tx.label} className="flex items-center justify-between py-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-slate-700">{tx.label}</span>
                      </div>
                      <span className={`text-sm font-semibold font-display ${tx.amount.startsWith('+') ? 'text-emerald-600' : 'text-slate-700'}`}>
                        {tx.amount}
                      </span>
                    </div>
                  ))}
              ...
              <div className="glass-card absolute -top-5 -right-5 rounded-xl shadow-lg px-4 py-3 flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-sm font-bold text-slate-800">Ver gráfico</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="py-24 bg-white">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { icon: BarChart3, title: 'Dashboard visual', desc: 'Cards com saldo, receitas e despesas do mês. Visão consolidada em segundos.', bg: 'bg-blue-50', ic: 'text-blue-600' },
              { icon: ShieldCheck, title: 'Controle de gastos', desc: 'Registre e edite transações com categoria, descrição e data. Histórico completo.', bg: 'bg-emerald-50', ic: 'text-emerald-600' },
              { icon: PieChart, title: 'Gráficos por categoria', desc: 'Visualize onde seu dinheiro vai com gráficos de pizza interativos por categoria.', bg: 'bg-violet-50', ic: 'text-violet-600' },
              { icon: FileDown, title: 'Exportar CSV', desc: 'Baixe suas transações filtradas em CSV para análise em planilhas externas.', bg: 'bg-amber-50', ic: 'text-amber-600' },
            ].map(({ icon: Icon, title, desc, bg, ic }) => (
              <div key={title} className="feature-card bg-white rounded-2xl p-6 border border-slate-100 shadow-sm">
      ...
      <section className="py-24 bg-[#f8fafc]">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <p className="text-sm font-semibold text-blue-600 uppercase tracking-widest mb-3">Como funciona</p>
          <h2 className="font-display text-4xl font-bold text-slate-900 tracking-tight mb-16">Simples como deve ser</h2>
          <div className="grid md:grid-cols-3 gap-10">
            {[
              ...
              <div key={step}>
                <div className="font-display text-7xl font-extrabold text-blue-50 leading-none mb-2 select-none">{step}</div>
                <h3 className="font-display text-xl font-bold text-slate-900 mb-2 -mt-5">{title}</h3>
      ...
      <footer className="border-t border-slate-100 py-10">
        <div className="max-w-6xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-4">
          <Link href="/" className="flex items-center gap-2">
            ...
            <span className="font-display font-bold text-slate-800">Finanza</span>
          </Link>
          ...
          <div className="flex gap-4 text-sm text-slate-400">
            ...
            <Link href="/register" className="hover:text-slate-700 transition-colors">Cadastrar</Link>
          </div>
        </div>
      </footer>
    </div>
